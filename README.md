...
| **Activity** | Add and split transactions, record refunds, schedule repeats, search by tags, and review CSV imports and suspected duplicates.       |
| **Budget**   | Set allocations, cover an overage, prepare for irregular bills, and choose fresh or rollover categories.                             |
| **Calendar** | Set a real payday, move between months, see planned repeating entries and months with extra cheques, and fill in missing bill dates. |
...
- **Repeats stay honest:** an Activity entry can project weekly, biweekly, monthly, quarterly, or yearly dates on the Calendar. Projected entries do not change spending, income, or balances until recorded.
