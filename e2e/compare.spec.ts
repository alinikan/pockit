...
test('desktop sidebar, sections, and theme remain laid out', async ({ page }) => {
  ...
  await page.getByRole('tab', { name: /Plan vs spent/ }).click()
  await expect(page.getByRole('heading', { name: 'Plan vs spent' })).toBeVisible()
...
test('long user-entered category names and large amounts do not break the phone layout', async ({
  ...
  await page.getByLabel('Amount to plan each period').fill('12345678.90')
  await noPageOverflow(page)
